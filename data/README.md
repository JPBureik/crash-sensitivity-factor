...
**Fetch command (to create a new snapshot)**
```bash
python scripts/get_sp500_tickers.py --out data/universe/sp500_$(date +%F).csv
```
...
python scripts/get_prices.py   --universe data/universe/sp500_2025-10-27.csv   --start 2005-01-01 --end 2025-10-31 --out data/raw/

# Build daily returns (light winsorization to catch bad prints)
python scripts/make_returns.py   --raw data/raw/ohlcv_yf_2025-10-31.parquet   --out data/returns/returns.parquet
```
...
```text
data/
  universe/      sp500_2025-10-27.csv           # committed (frozen list)
...
```bash
# 1) Universe (skip if the committed snapshot exists)
python scripts/get_sp500_tickers.py --out data/universe/sp500_$(date +%F).csv
