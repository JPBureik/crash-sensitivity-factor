...
def test_joint_cumulant4_center_true_and_general():
    ...
    expected_center = E(A*B*C*D) - E(A*B)*E(C*D) - E(A*C)*E(B*D) - E(A*D)*E(B*C)
...
def test_rolling_beta_series_basic_for_both_metrics():
    ...
    ri = pd.Series(0.2*(rm**2 - rm.mean()**2) + 0.1*rng.normal(size=len(idx)), index=idx)

    # coskew
    out3 = rolling_beta_series(ri, rm, window=10, min_obs=5, which="coskew",
                               cfg=CoMomentConfig(winsor=0.0, robust_scale=False, shrink_tau=0.0))
    # cokurt
    out4 = rolling_beta_series(ri, rm, window=10, min_obs=5, which="cokurt",
                               cfg=CoMomentConfig(winsor=0.0, robust_scale=False, shrink_tau=0.0))
...
def test_panel_betas_shape_and_columns():
    ...
    out = panel_betas(r, rm, window=8, min_obs=4, which="coskew",
                      cfg=CoMomentConfig(winsor=0.0, robust_scale=False, shrink_tau=0.0))
...
def test_scale_invariance_with_robust_scale():
    ...
    assert np.isfinite(b3) and np.isfinite(b3_scaled) and np.isfinite(b4) and np.isfinite(b4_scaled)
