...
from crashload.signal import CoMomentConfig, crash_score
...
def test_crash_score_nonmonthly_z_logic(monkeypatch):
    ...
    out = crash_score(
        r, rm, window=5, min_obs=3, lambda_=0.5, cfg=CoMomentConfig(), monthly=False
    )
...
def test_crash_score_monthly_resample_and_lambda_weight(monkeypatch):
    ...
    def fake_panel_betas(r_, rm_, window, min_obs, which="coskew", cfg=None):
        base = pd.DataFrame(index=r_.index, columns=r_.columns, dtype=float)
        vals = [1.0, 2.0, 3.0]
        for col, v in zip(base.columns, vals, strict=True):
            base[col] = v
        return base
    ...
    out = crash_score(
        r, rm, window=5, min_obs=3, lambda_=0.5, cfg=CoMomentConfig(), monthly=True
    )
    ...
    scale = 1.4826
    z3 = (pd.Series([-1.0, -2.0, -3.0], index=["A", "B", "C"]) - (-2.0)) / scale
    z4 = (pd.Series([1.0, 2.0, 3.0], index=["A", "B", "C"]) - (2.0)) / scale
